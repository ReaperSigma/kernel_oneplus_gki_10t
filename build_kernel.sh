...
get_proton_clang-13.0() {
	...
	TC="$TOOLCHAIN_DIR/clang-r416183b"
...
build() {
	...
	run "make CC='ccache clang-12' CXX='ccache clang++-12' LLVM=1 LLVM_IAS=1 LD=ld.lld-12 HOSTLD=ld.lld-12 AR=llvm-ar-12 AS=llvm-as-12 STRIP=llvm-strip-12 OBJCOPY=llvm-objcopy-12 OBJDUMP=llvm-objdump-12 NM=llvm-nm-12 HOSTAR=llvm-ar-12 HOSTAS=llvm-as-12  READELF=llvm-readelf-12  O=out $KERNEL_CONFIG"
	run "make CC='ccache clang-12' CXX='ccache clang++-12' LLVM=1 LLVM_IAS=1 LD=ld.lld-12 HOSTLD=ld.lld-12 AR=llvm-ar-12 AS=llvm-as-12 STRIP=llvm-strip-12 OBJCOPY=llvm-objcopy-12 OBJDUMP=llvm-objdump-12 NM=llvm-nm-12 HOSTAR=llvm-ar-12 HOSTAS=llvm-as-12 READELF=llvm-readelf-12  O=out -j8"
	run "make CC='ccache clang-12' CXX='ccache clang++-12' LLVM=1 LLVM_IAS=1 LD=ld.lld-12 HOSTLD=ld.lld-12 AR=llvm-ar-12 AS=llvm-as-12 STRIP=llvm-strip-12 OBJCOPY=llvm-objcopy-12 OBJDUMP=llvm-objdump-12 NM=llvm-nm-12 HOSTAR=llvm-ar-12 HOSTAS=llvm-as-12 READELF=llvm-readelf-12  O=out $KERNEL_CONFIG ARCH=$TARGET_ARCH INSTALL_MOD_PATH=\"$BUILD_MODULES_DIR\" INSTALL_MOD_STRIP=1 modules_install"
	if [[ $BUILD_HAS_DTBO == "1" ]]; then
		make_dtboimg
	fi
